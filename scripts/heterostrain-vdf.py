...
def ask_user():
    ...
    #return False, 0.32, 0.5, 0.247, None
    ...
    if not is_hbl:
        ...
        if given_delta:
            ...
            a = np.min([a1,a2])
            delta = ( np.max([a1,a2]) - np.min([a1,a2]) ) / a 
...
def computeAllTris(triangles, centers, pixelsize, poissonratio, a, delta, is_hbl, given_twist, manual=False):
    ...
    return tri_centers, thetas, het_strains, area_fracts

def hetstrain_from_adjacencies_noplot(centers, adjacency_table, is_hbl=None, a=None, pixelsize=None, poissonratio=None, delta=None, given_twist=None):
    if is_hbl is None : is_hbl, a, pixelsize, poissonratio, delta, given_twist = ask_user()
    triangles = get_triangles(adjacency_table)
    tri_centers, thetas, het_strains, area_fracts = computeAllTris(triangles, centers, pixelsize, poissonratio, a, delta, is_hbl, given_twist)
    theta_colors, het_strain_colors = getColors(thetas, het_strains)  
    return triangles, thetas, het_strains, theta_colors, het_strain_colors

def hetstrain_from_adjacencies(centers, adjacency_table, ax1, ax2):
    ...
    if is_hbl and given_twist is None:
        plotTriangleQuantity(triangles, tri_centers, thetas, theta_colors, ax1, '$<\\theta_m> = {:.2f}^o$'.format(np.nanmean(thetas)), centers)
        plotTriangleQuantity(triangles, tri_centers, het_strains, het_strain_colors, ax2, '$<deform> = {:.2f}\%$'.format(np.nanmean(het_strains)), centers)
        return np.nanmean(thetas), np.nanmean(het_strains), is_hbl, a, pixelsize, poissonratio, delta, given_twist
    elif is_hbl and given_twist is not None:
        plotTriangleQuantity(triangles, tri_centers, thetas, theta_colors, ax1, '$<\delta> = {:.2f}\% ({:.2f}\%)$'.format(np.nanmean(thetas), 100*delta), centers)
        plotTriangleQuantity(triangles, tri_centers, het_strains, het_strain_colors, ax2, '$<deform> = {:.2f}\%$'.format(np.nanmean(het_strains)), centers)
        return given_twist, np.nanmean(het_strains), is_hbl, a, pixelsize, poissonratio, delta, given_twist
    elif not is_hbl:
        plotTriangleQuantity(triangles, tri_centers, thetas, theta_colors, ax1, '$<\\theta_m> = {:.2f}^o$'.format(np.nanmean(thetas)), centers)
        plotTriangleQuantity(triangles, tri_centers, het_strains, het_strain_colors, ax2, '$<\epsilon> = {:.2f}\%$'.format(np.nanmean(het_strains)), centers)
        return np.nanmean(thetas), np.nanmean(het_strains), is_hbl, a, pixelsize, poissonratio, delta, given_twist
