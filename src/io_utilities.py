...
class DataSetContainer:
    ...
    def __init__(self, folderprefix, dataset_number = None):
        ...
        # used to call a user query if the sample rotation is unset since need this
    ...
    def set_sample_rotation(self):
        if self.check_parameter_is_set("SampleRotation"):
            return 
        elif not self.check_has_diskset():
            print('dataset has no diskset file nor a specified sample rotation, unable to proceed')
            exit(1)
        else:
            diskset = self.extract_diskset()
            g = diskset.rotated_normgset(sanity_plot=True)
            g = diskset.clean_normgset(sanity_plot=True)
            exit(1)
